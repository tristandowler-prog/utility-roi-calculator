...
st.markdown(
    f'''
    <div style="padding-bottom: 24px; color: {MUTED}; font-size: 1.05rem;">
...
def vertical_inputs(prefix):
    ...
    with c1:
        ...
        people_per_team = st.number_input(
            "People Per Team",
            min_value=1,
            value=3,
            key=f"{prefix}_p_per_t"
        )

        field_hours = st.number_input(
            "Field Hours Per Event",
            min_value=1,
            value=40,
            key=f"{prefix}_field_hours"
        )

        avg_wage = st.number_input(
            "Avg Wage ($/hr)",
            min_value=10.0,
            value=75.0,
            key=f"{prefix}_wage"
...
def calculate(data):
    ...
    optimized_aviation = aviation * (
        1 - aviation_reduction
    )

    optimized_logistics = logistics * (
        1 - logistics_reduction
    )
...
for idx, tab in enumerate(tabs):
    ...
    with tab:
        ...
        with m1:
            st.metric(
                "Per Event Savings",
                f"{currency_symbol}{result['savings']:,.0f}"
            )
        ...
        with m3:
            payback = (
                subscription / result["savings"]
                if result["savings"] > 0 else 0
            )
...
<div style="padding: 28px; background-color: {CARD}; border: 1px solid {BORDER}; border-radius: 16px;">
    ...
    <div style="font-size: 0.85rem; letter-spacing: 1px; text-transform: uppercase; color: {MUTED}; margin-bottom: 12px;">
        Executive Summary
    </div>

    <div style="font-size: 2.2rem; font-weight: 700; color: white; margin-bottom: 10px;">
        {currency_symbol}{net_dividend:,.0f}
    </div>

    <div style="color: {MUTED}; font-size: 1rem;">
...
* **Field Labor Liability:** Calculated as $L = (T \\times H \\times (P \\times W))$, where $T$ is the number of teams, $H$ is total field hours per event, $P$ is personnel per team, and $W$ is the hourly wage.
